...
Let:
  u = v_w  (centre embedding, shape: emb_dim)
  v = v_c  (positive context embedding)
  n_k      (negative context embeddings, shape: K × emb_dim)
...
Gradient w.r.t. input (centre) embedding:
  dL/du = -(1 - σ(v*u)) * v  +  Σ_k σ(n_k*u) * n_k

Key insight: only 1 + K rows of W_out and 1 row of W_in are touched per sample.
"""

import numpy as np

def compute_gradients(
    v_w: np.ndarray,        # centre embedding  (emb_dim,)
    v_c: np.ndarray,        # positive context  (emb_dim,)
